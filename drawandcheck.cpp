...
void RenderScrollingBackground(std::vector <double>& offsetSpeed,const int acceleration,
	...
	SDL_Renderer* gRenderer)
{
	...
	{
		offsetSpeed[i] +=BACKGROUND_SPEED +acceleration;
		if (offsetSpeed[i] >gBackgroundTexture[i].GetHeight())
		{
			offsetSpeed[i] = 0;
		}
		gBackgroundTexture[i].Render(0,offsetSpeed[i],  gRenderer);
		gBackgroundTexture[i].Render(0,offsetSpeed[i] -gBackgroundTexture[i].GetHeight(), gRenderer);
	}
}
